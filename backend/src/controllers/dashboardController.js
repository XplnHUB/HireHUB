...
export const getDashboardActivity = async (req, res) => {
  try {
    ...
    const activity = recentApplications.map(app => ({
      ...
      description: `at ${app.job.recruiter.companyName}`,
...
export const getDashboardInterviews = async (req, res) => {
  try {
    // Mock interviews for now as schema might be complex
    const interviews = [
      { day: '12', time: '10:00 AM', company: 'TechCorp', role: 'Frontend Dev', type: 'Video' },
      { day: '15', time: '2:00 PM', company: 'StartupHub', role: 'Full Stack', type: 'Onsite' }
    ];
    res.json(interviews);
  } catch (error) {
    console.error('Error getting dashboard interviews:', error);
